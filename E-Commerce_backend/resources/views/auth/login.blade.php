...
@section('content')
    <div
        class="min-h-screen bg-gradient-to-br from-[#22c55e] to-[#16a34a] flex items-center justify-center p-4 sm:p-6 lg:p-8">

        {{-- Decorative circles --}}
        <div
            class="fixed top-0 left-0 w-[500px] h-[500px] rounded-full bg-white/10 -translate-x-1/4 -translate-y-1/4 pointer-events-none">
        </div>
        <div
            class="fixed bottom-0 right-0 w-[600px] h-[600px] rounded-full bg-white/[0.07] translate-x-1/3 translate-y-1/3 pointer-events-none">
        </div>
        <div
            class="fixed top-1/3 right-0 w-[300px] h-[300px] rounded-full bg-white/[0.05] translate-x-1/2 pointer-events-none">
        </div>
        <div
            class="fixed bottom-1/4 left-0 w-[250px] h-[250px] rounded-full bg-white/[0.06] -translate-x-1/2 pointer-events-none">
        </div>
        ...
        <div
            ...
            <div
                ...
                <div class="flex-1 flex items-center justify-center py-6">
                    <svg viewBox="0 0 400 400" class="w-full max-w-[320px] h-auto">
                        ...
                        <path d="M180 200 V185 C180 172 188 165 200 165 C212 165 220 172 220 185 V200" fill="none"
                            stroke="white" stroke-width="5" stroke-linecap="round" opacity="0.95" />
                        ...
                        <path d="M302 100 L308 108 L320 93" fill="none" stroke="white" stroke-width="3"
            ...
            <div class="p-8 lg:p-12 xl:p-16 flex flex-col justify-center">
                ...
                @if ($errors->any())
                    <div
                        class="mt-6 p-3 bg-red-50 border border-red-200 rounded-[10px] text-sm text-red-600 font-medium text-center">
                        {{ $errors->first() }}
                    </div>
                @endif
                ...
                @if (session('status'))
                    <div
                        class="mt-6 p-3 bg-emerald-50 border border-emerald-200 rounded-[10px] text-sm text-emerald-600 font-medium text-center">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login.submit') }}" class="mt-8 space-y-5" novalidate>
                    ...
                    <div>
                        <div class="relative group">
                            <span
                                class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400 group-focus-within:text-[#22c55e] transition-colors duration-300">
