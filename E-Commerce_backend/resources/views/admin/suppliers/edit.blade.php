...
<div class="container-fluid p-0">
    <div class="card border-0 shadow-sm rounded-4" style="max-width: 760px;">
        <div class="card-header bg-white py-3 rounded-4 d-flex flex-wrap align-items-center gap-2">
            <div>
                <h5 class="fw-bold mb-0"><i class="bi bi-pencil-square me-2 text-success"></i>{{ __('Edit Supplier') }}</h5>
                <small class="text-muted">{{ $supplier->name }}</small>
            </div>
            <a href="{{ route('admin.suppliers.show', $supplier) }}" class="btn btn-outline-secondary btn-sm ms-auto">
            ...
            <form action="{{ route('admin.suppliers.update', $supplier) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-3 pt-1">
                            <div class="form-check">
                                <input type="radio" name="status" value="1" class="form-check-input" id="statActive" {{ old('status', $supplier->status) ? 'checked' : '' }}>
                                <label class="form-check-label small" for="statActive">{{ __('Active') }}</label>
                            </div>
                            <div class="form-check">
                                <input type="radio" name="status" value="0" class="form-check-input" id="statInactive" {{ !old('status', $supplier->status) ? 'checked' : '' }}>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-success px-4">
                        <i class="bi bi-check-lg me-1"></i>{{ __('Update Supplier') }}
                    </button>
                    <a href="{{ route('admin.suppliers.show', $supplier) }}" class="btn btn-outline-secondary px-4">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
