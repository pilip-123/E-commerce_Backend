    ...
    <script>
        (function() {
            ...
            function bc(id, items, vk, lk, cl, ev) {
                ...
                html +=
                    '</div><div style="display:flex;align-items:flex-end;gap:3px;height:100%;position:relative;z-index:1;">';
                items.map(function(it, i) {
                    var h = it[vk] / m * 100;
                    ...
                    var shadow = cl === 'g' ? '0 2px 8px rgba(5,150,105,.25)' : cl === 'p' ?
                        '0 2px 8px rgba(4,120,87,.3)' : '0 2px 8px rgba(16,185,129,.25)';
                    ...
                    html += '<div title="' + lb + ': ' + it[vk] + '" style="width:70%;max-width:36px;height:' +
                        h + '%;border-radius:5px 5px 2px 2px;background:' + grad + ';box-shadow:' + shadow +
                        ';min-height:2px;transition:all .25s ease;cursor:pointer;" onmouseover="this.style.opacity=\'.8\'" onmouseout="this.style.opacity=\'1\'"></div>';
                    if (i % e === 0) html +=
                        '<span style="position:absolute;bottom:-24px;left:50%;transform:translateX(-50%);font-size:.6rem;font-weight:600;color:var(--bs-secondary-color);white-space:nowrap;">' +
                        lb + '</span>';
                    html += '</div>';
                });
                html += '</div>';
